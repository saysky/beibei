...
import com.liuyanzhao.sens.model.dto.Theme;
...
public class SensUtils {
    ...
    public static String getRss(List<Post> posts) throws FeedException {
        ...
        for (Post post : posts) {
            ...
            item.setLink(SensConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "/article/" + post.getPostId());
    ...
    public static String getSiteMap(List<Post> posts) {
        ...
        for (Post post : posts) {
            urlItem = "<url><loc>" + urlPath + post.getPostId() + "</loc><lastmod>" + getStringDate(post.getPostDate(), "yyyy-MM-dd'T'HH:mm:ss.SSSXXX") + "</lastmod>" + "</url>";
            urlBody += urlItem;
        }
